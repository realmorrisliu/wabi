...
|---|---|---|
| `research-plan` | openai-codex/gpt-5.6-sol | read-only Research & Plan — deep-dives complex or uncertain tasks and returns an implementation plan (max thinking) |
...
```ts
subagent({ agent: "research-plan", task: "research and plan the multi-file refactor" })   // foreground: complex/uncertain task — deep dive + plan (read-only)
subagent({ agent: "research-plan", task: "research and plan the auth rework", background: true })  // background: read-only only
...
Routing policy lives in the subagent tool's guidelines and the `subagent-orchestration` skill: the parent agent itself implements (non-atomic implementation stays in the parent); before a complex or uncertain task it delegates Research & Plan to the read-only `research-plan` agent — the deep dive and the plan — then implements the adopted plan; `reviewer` runs an independent correctness + complexity pass after risky changes; `creative-worker` builds visual/interactive artifacts. Use multiple sibling `subagent` calls in one assistant message for independent blocking work. Wabi permits at most four concurrent children, rejects a second write-capable child while one is active, and rejects write-capable children in the background (background is read-only only). Sequential composition happens naturally as each result is handed back to the parent.
...
`/review` defaults to the current working tree. `/research-plan` runs the read-only research-plan agent (foreground by default) — it researches the task and returns the plan, never implements. `/review` may run in the background. For creative builds, ask the parent to delegate to `creative-worker`.
